...
def autocolor(color,_typ):
    if isinstance (color,str) and color.startswith('#') and len(color)==7:
        ...
        (r,g,b)=color[0:3]
        if _typ=='str':
            return '#'+"".join(map(chr, (int(r*255),int(g*255),int(b*255)))).encode('hex')
        elif _typ=='float':
            return (r,g,b)
        elif _typ=='int':
        ...
        if _typ=='str':
            ...
            return(r/255.0,g/255.0,b/255.0)
    elif isinstance (color, wx._gdi.Colour) and isinstance(color[0],int) and max(color)<=255:
        #got a float tupple
        (r,g,b)=color[0:3]
        if _typ=='str':
            return '#'+"".join(map(chr, (r,g,b))).encode('hex')
        ...
        #unknown color format. return black
        print color
        print type(color)
        if _typ=='str':
            return '#000000'
    ...
    def __init__(self,windowtitle, entries):
        ...
        for entry in entries:
            ...
            if entry[0]=='wxnotebook':
                ...
                #self.widgets[-1].SetColour(wxcolorfrom(_default,_typ))
                self.widgets[-1].SetColour(autocolor(_default,'str'))
                self.sizer.Add(wx.StaticText(self.panel,label=_label),1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
                self.sizer.Add(self.widgets[-1], 1, wx.ALL|wx.CENTER|wx.EXPAND|wx.ALIGN_CENTER_VERTICAL, 5)
            elif entry[0]=='wxdate':
                self.widgets.append(wx.DatePickerCtrl(self.panel))
                ...
                self.widgets.append(wxcal.CalendarCtrl(self.panel))
                dt=wx.DateTime().Today()
                dt.SetYear(int(_default.split('-')[0]))
                dt.SetMonth(int(_default.split('-')[1])-1)
                dt.SetDay(int(_default.split('-')[2]))
                ...
                self.widgets.append(wx.wx.CheckListBox(self.panel,choices=_range.split('|')))
                #try co convert _default to list of int
                if _default.split('|')[0].isdigit():
                    indexes = [int(i) for i in _default.split('|')]
                    self.widgets[-1].SetChecked(indexes)
                else:
    ...
    def GetValues(self):
        ...
        for idx in range(0,len(self.widgets)):
            ...
            if 'Panel' in str(type(_w)):
                ...
                # for some weird reason,on mac-OSX, _w.GetColour sometimes return an HTML color string
                # whereas an integer tupple is returned on windows and unices
                self.response.append(autocolor(_w.GetColour(),_typ))
                #self.response.append(wxcolorto((_w.GetColour()[0], _w.GetColour()[1], _w.GetColour()[2]),_typ))
                # for some weird reason,on mac-OSX, _w.GetColour sometimes return an HTML color string
                #if 'str' in str(type(_w.GetColour())):
                #    self.response.append(_w.GetColour())
                #elif 'wx._gdi.Colour' in str(type(_w.GetColour())):
                #    self.response.append('#%02x%02x%02x' % (_w.GetColour()[0], _w.GetColour()[1], _w.GetColour()[2]))
                #else:
                #    self.response.append(_w.GetColour().GetAsString(wx.C2S_HTML_SYNTAX))
                ...
                print "Don't know how to convert "+str(type(_w))
