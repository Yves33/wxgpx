...
("Open street maps",            "http://c.tile.openstreetmap.org/{z}/{x}/{y}.png","openstreetmaps"),\
...
class DownloadThread:
    ...
    def DownloadTile(self, tile):
        ...
        try:
            ...
            downloader=mozurllib()
            url=downloader.retrieve(url, filename)
            return url
        except IOError as e:
            return False
...
class WxMapImage(object):
    ...
    def __makebuffers(self):
        ...
        if self.img.IsOk():
            ...
            a=self.img.GetAlphaData()
            if self.hasalpha:
                self.pixdata=''
                for i in range(len(rgb)/3):
                    self.pixdata+=rgb[3*i:3*i+3]+a[i]
            else:
    ...
    def FromBase64(cls,b64):
        obj=cls.__new__(cls)
        obj.img=wx.ImageFromStream(cStringIO.StringIO(base64.b64decode(b64)))
        obj.__makebuffers()
        return obj
...
class WxDCArtist(wx.Panel):
    def __init__(self, *args, **kwargs):
        ...
        self._mainbuffer=wx.EmptyBitmapRGBA(32,32)
    ...
    def RGBALines(self,vertices):
        dc=wx.GCDC(wx.MemoryDC(self._curbuffer))
        self._preparedc(dc)
        for i in xrange(0,len(vertices)/6-1):
                dc.SetPen(wx.Pen(wx.Colour(vertices[i*6+2]*255,vertices[i*6+3]*255,vertices[i*6+4]*255,vertices[i*6+5]*255),self.pensize))
    ...
    def _preparedc(self,dc):
        dc.SetPen(wx.Pen(wx.Colour(self.pencolor[0]*255,\
                                self.pencolor[1]*255,\
                                self.pencolor[2]*255,\
                                self.pencolor[3]*255),\
                                self.pensize))
        dc.SetBrush(wx.Brush(wx.Colour(self.brushcolor[0]*255,\
                                    self.brushcolor[1]*255,\
                                    self.brushcolor[2]*255,\
                                    self.brushcolor[3]*255)))
        dc.SetTextForeground(wx.Colour(self.pencolor[0]*255,\
                                self.pencolor[1]*255,\
                                self.pencolor[2]*255,\
                                self.pencolor[3]*255))
        dc.SetTextBackground(wx.Colour(self.brushcolor[0]*255,\
                                    self.brushcolor[1]*255,\
                                    self.brushcolor[2]*255,\
                                    self.brushcolor[3]*255))
    ...
    def MakeBuffer(self):
        ...
        return wx.EmptyBitmapRGBA(width,height)

    def SizeBuffer(self,buff):
        return wx.EmptyBitmapRGBA(self.width,self.height)
    ...
    def StartDrawing(self):
        ...
        self._mainbuffer=wx.EmptyBitmapRGBA(self.width, self.height)
        dc=wx.GCDC(wx.MemoryDC(self._mainbuffer))
        dc.SetBackground(wx.Brush(wx.Colour(0.0,0.0,0.0,255.0)))
        dc.Clear()
...
class WxMapBase(wx.Panel):
    def __init__(self, parent,*args, **kwargs):
        ...
        self.layerorder=1
        #create cursors
        self.stdcursor=wx.StockCursor(wx.CURSOR_ARROW)
        self.handcursor=wx.StockCursor(wx.CURSOR_BULLSEYE)
    ...
    def LoadProviders(self, path):
        try:
            ...
            print "unable to open map providers"
    ...
    def Draw(self,updatebuffers=True):
        ...
        try:
            ...
            print "could not set context!! maybe next time"
            return
        if not self.inited:
