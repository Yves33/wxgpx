...
if __name__ == "__main__":
    class MainFrame(wx.Frame):
        def __init__(self, parent, id, title, size=(500,500)):
            ...
            # map widget
            self.mapwidget=wxmapwidget.WxMapWidget(vsplitter,usegl=self.config.getboolean("map","map_usegl"),localcache=map_cache,style=wx.BORDER_SUNKEN)
            ...
            pub.subscribe(self.OnSigCurChanged, "CurChanged")                   # we need to be aware of current point
            pub.subscribe(self.OnSigSelChanged, "SelChanged")                   # we need to be aware of selection
            pub.subscribe(self.OnSigValChanged, "ValChanged")                   # not used!
            self.Show()
        ...
        def SaveFile(self,filename):
            if filename[-4:]=='.npz' or filename[-4:]=='.NPZ':
                ...
                (fields,save_enabled)=WxQuery("GPX export dialog",[('wxchecklist','Choose fields to export','|'.join(allowedfields),'time|speed','str'),
        ...
        def OnUnitsMenu(self,event):
            ...
            for head in self.gpx.get_header_names():
                if not head.startswith('_'):
                    ...
                    i+=1
            pub.sendMessage("ValChanged",arg1=self.id)
            self.Refresh()
        ...
        def OnReplayTimer(self,event):
            ...
            while not self.gpx['ok'][self.idx]:
                ...
                if self.idx > self.selstop:
                    self.idx=self.selstart
            pub.sendMessage("CurChanged",arg1=self.id,arg2=self.idx)
    ...
    class DemoApp(wx.App):
        ...
        def OnInit(self):
            ...
            if os.path.exists(splash_file):
                ...
                splash=wx.SplashScreen(splash_image.ConvertToBitmap(),wx.SPLASH_CENTRE_ON_SCREEN|wx.SPLASH_TIMEOUT,1000,None, -1)
                splash.Show()
